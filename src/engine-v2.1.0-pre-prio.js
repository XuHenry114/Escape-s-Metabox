...
(function (root, factory) {
  ...
  Engine.prototype._canPush = function (g, chain, dir) {
    ...
    if (!inGrid(nr, nc)) {
      ...
      if (this.options.pushOut !== false && g !== this.root && chain.length === 1) {
        var lastBoxC = chain[0];
        if (lastBoxC.type === '0B') {
          var candC = this._collectTwinCandidates(lastBoxC.spaceId);
          var hasTwinC = false;
          for (var ti = 0; ti < candC.live.length; ti++) { if (candC.live[ti].box !== lastBoxC) { hasTwinC = true; break; } }
          if (!hasTwinC && candC.base.length) hasTwinC = true;
          if (hasTwinC) return false;   // 同型实例存在 → 穿梭优先（走进入/穿梭判定）
        }
  ...
  Engine.prototype._resolveLanding = function (view, meta, dir, twinEntry, extra) {
    ...
    if (l2) return l2;
    if (!(extra && extra.tunnel) && extra && extra.boxType === '0B' && !extra.selfLoop) return this._checkLandingCell(view, idToRC(meta.ac), dir);
    return null;
  };
  ...
  Engine.prototype._step = function (dir) {
    ...
    // —— 链进入（用户规则1 定版）：无延迟一拍，链 ≥2 且推不动 → 直接进入判定（最远合法盒 / 盒进入盒）——
    this._enterWarm = null;

    // —— 链进入（用户规则1）：找最远合法盒 B(k)（对应边缘正中非墙），由其【前面项目 P】进入 ——
    //    P 是 AC → AC 进入 B(k)（现有）；P 是盒 → P 缩入 B(k)（P 实体移入 B(k) 空间开口格；AC 移至 P 原格；不切视图）
    var bestK = -1;
    for (var k2 = chain.length - 1; k2 >= 0; k2--) {
      var bS = chain[k2];
      var mE = this.spaceMeta[String(bS.spaceId)];
      if (!mE) continue;
      var vE = this._spaceView(String(bS.spaceId), mE.type);
      if (!vE) continue;
      var lE = this._resolveLanding(vE, mE, dir, null, { boxType: bS.type });
      if (lE) { bestK = k2; break; }
    }
    if (bestK >= 0) {
      var farBox = chain[bestK];
      if (bestK > 0) {
        // —— P = chain[bestK-1]（盒）缩入 farBox ——
        var prevBox = chain[bestK - 1];
        var mP = this.spaceMeta[String(prevBox.spaceId)];
        var vP = this._spaceView(String(prevBox.spaceId), mP ? mP.type : 'MLE');
        var lP = vP ? this._resolveLanding(vP, mP, dir, null, { boxType: prevBox.type }) : null;
        var lFree = (lP && lP.land) || null;
        // P 实体：从当前网格移除
        this._snapshot();
        var fromCellP = this.current.grid[prevBox.pos.r][prevBox.pos.c];
        if (fromCellP) fromCellP.box = null;
        if (lFree && vP) {
          // P 放入 farBox 空间视图（_spaceView 的新共享层=farBox 内容），落在开口格
          var pView = this._spaceView(String(farBox.spaceId), (this.spaceMeta[String(farBox.spaceId)] || {}).type);
          if (pView && pView.grid[lFree.r][lFree.c]) {
            pView.grid[lFree.r][lFree.c].box = prevBox;
            prevBox.pos = { r: lFree.r, c: lFree.c };
          } else if (pView) {
            pView.grid[lFree.r][lFree.c] = { box: prevBox, baseLabel: 'WA' };
            prevBox.pos = { r: lFree.r, c: lFree.c };
          }
        }
        // AC 移至 P 原格（AC 视点不变：仍在当前空间）
        this.pos = clonePos(prevBox.pos ? prevBox.pos : clonePos(chain[bestK - 1].pos));
        this.moves++;
        this._ev('boxEnter', { dir: dir, enterer: prevBox.uid, target: farBox.uid, targetSpace: +farBox.spaceId });
        return true;
      }
      var isSelfA = (farBox.type === '0B' && String(farBox.spaceId) === String(this.current.spaceId));
      var resA = this._tryEnter(farBox, dir, { chainLen: chain.length, boxType: farBox.type, selfLoop: isSelfA });
      if (resA) return true;
    }
    // —— 链末越界：进入失败后 → ①同型另一实例则穿梭 ②推出链末盒（无开口/不可入）；其余项目前进一格 ——
    var d0 = DIRS[dir];
    var lastB = chain[chain.length - 1].pos;
    var nr0 = lastB.r + d0.dr, nc0 = lastB.c + d0.dc;
    if (!inGrid(nr0, nc0)) {
      var lastChainBox = chain[chain.length - 1];
      // —— 规则3 穿梭：链末盒存在同型另一 0B 实例 → 穿梭进入其空间 ——
      if (lastChainBox.type === '0B') {
        var candSh2 = this._collectTwinCandidates(lastChainBox.spaceId);
        var shBox = null, shGrid = null;
        for (var si2 = 0; si2 < candSh2.live.length; si2++) {
          if (candSh2.live[si2].box !== lastChainBox) { shBox = candSh2.live[si2].box; shGrid = candSh2.live[si2].grid; break; }
        }
        if (shBox) {
          var resSh2 = this._tryEnter(shBox, dir, { chainLen: chain.length, boxType: shBox.type, shuttle: true, shuttleFrom: lastChainBox.uid });
          if (resSh2) {
            // 其余项目（farBox 之前链项）前进一格（AC 已随 _tryEnter 进入 shuttle 实例）
            var d2d = DIRS[dir];
            for (var qj = chain.length - 2; qj >= 0; qj--) {
              var qb2 = chain[qj], qfrom2 = clonePos(qb2.pos);
              var qto2 = { r: qfrom2.r + d2d.dr, c: qfrom2.c + d2d.dc };
              var qfcell2 = this.current.grid[qfrom2.r] && this.current.grid[qfrom2.r][qfrom2.c];
              if (qfcell2) qfcell2.box = null;
              var qtcell2 = this.current.grid[qto2.r] && this.current.grid[qto2.r][qto2.c];
              if (!qtcell2) { if (!this.current.grid[qto2.r]) this.current.grid[qto2.r] = []; this.current.grid[qto2.r][qto2.c] = { box: null, baseLabel: 'WA' }; qtcell2 = this.current.grid[qto2.r][qto2.c]; }
              qtcell2.box = qb2;
              qb2.pos = qto2;
            }
            this._ev('shuttle', { dir: dir, fromBoxUid: lastChainBox.uid, toBoxUid: shBox.uid, spaceId: +lastChainBox.spaceId });
            return true;
          }
        }
      }
      var pushOut = this._canPushOut(chain[chain.length - 1], dir);
      if (pushOut) {
        var boxOut = chain[chain.length - 1];
        this._snapshot();
        ...
        // 其余项目（链中前部盒）前进一格
        for (var pi = chain.length - 2; pi >= 0; pi--) {
          var pb = chain[pi], pfrom = clonePos(pb.pos);
          var pto = { r: pfrom.r + d0.dr, c: pfrom.c + d0.dc };
          this.current.grid[pfrom.r][pfrom.c].box = null;
          var ptcell = this.current.grid[pto.r][pto.c];
          if (!ptcell) ptcell = this.current.grid[pto.r][pto.c] = { box: null, baseLabel: 'WA' };
          ptcell.box = pb;
          pb.pos = pto;
        }
        this.pos = t;
        this.moves++;
        this._ev('push', { dir: dir, chain: chain.length, boxUid: boxOut.uid, spaceId: +boxOut.spaceId, pushOut: true, toSpace: +pushOut.pg.spaceId, to: clonePos(boxOut.pos) });
        return true;
      }
    }
  ...
  Engine.prototype._displayCells = function (g, playerPos) {
    ...
    for (var id = 1; id <= CELLS; id++) {
      ...
      // 空地（全程仅玩家所在位置为 AC；其余 AC 卡显示 'WA'；盒卡原格（盒已推走/推出）显示 'WA'——原地为地板）
      var onPlayer = playerPos && playerPos.r === rc.r && playerPos.c === rc.c;
      var baseLab = cell.baseLabel || 'WA';
      if (onPlayer) baseLab = 'AC';                                                   // 仅玩家位置显示 AC
      else if (baseLab === 'AC') baseLab = 'WA';                                      // 原 AC 卡离开后 → WA
      else if (baseLab === 'MLE' || baseLab === 'TLE' || baseLab === 'UKE') baseLab = 'WA';   // 盒卡原格无盒 → 地板
      out.push({ id: id, r: rc.r, c: rc.c, label: baseLab, space: 0, size: '0B', uid: null, time: +g.spaceId, st: baseLab });
    }
    return out;
  };
