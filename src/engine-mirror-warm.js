...
(function (root, factory) {
  ...
  Engine.prototype._tryEnter = function (box, dir, extra) {
    ...
    var landing = this._resolveLanding(view, meta, dir, twinEntry, extra);
    if (!landing) {
    ...
    if (box.type === '1MB' && this.options.oneWayTwin) {
      ...
      if (selfLoop) {
        ...
        frame = { view: view, parent: parentGrid, boxUid: twin.uid, boxPos: clonePos(twin.pos), spaceId: spaceId, master: false };
        evs.push({ type: 'teleport', dir: dir, fromBoxUid: box.uid, fromSpaceId: +spaceId, toBoxUid: twin.uid, viewSpaceId: +spaceId, land: clonePos(landing.land) });
      }
    } else {
  ...
  Engine.prototype._exitAttempt = function (dir) {
    ...
    // —— 目标格为盒：优先同尺寸穿梭进入（题解：退出直面盒则直接进入，不推）——
    if (cell.box !== null) {
      ...
      if (myBox0.type === cell.box.type) return this._tryEnter(cell.box, dir, { tunnel: true, ownerGrid: boxGrid, exitPos: { r: this.pos.r, c: this.pos.c } });
      // 尺寸不同：交给下方推出/推链逻辑
    }
    ...
    if (this._canPush(boxGrid, chain, dir)) {
      this._snapshot();
      this._pushChain(boxGrid, chain, dir);
    ...
    if (myBox.type === chain[0].type) {
      return this._tryEnter(chain[0], dir, { tunnel: true, ownerGrid: boxGrid, exitPos: { r: this.pos.r, c: this.pos.c } });
    }
