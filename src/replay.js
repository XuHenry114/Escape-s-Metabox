...
function runReplay(engine, text, opts) {
  ...
  stat.visited = {};
  stat.visited[String(engine.getState().spaceId !== undefined && engine.getState().spaceId !== null ? engine.getState().spaceId : engine.getState().currentBox)] = true;   // 起点空间视为已访问
  var winShown = false;
  ...
  for (var ti = 0; ti < tokens.length; ti++) {
    ...
    if (tok.type === 'num') {
      ...
      var actual = st.spaceId !== undefined && st.spaceId !== null ? st.spaceId : st.currentBox;
    ...
    stat.steps++;
    // visited 采用"当前实际空间"（spaceId 优先；currentBox 为帧栈推断，teleport 宿主层时二者可分离）
    var visitedId = (st2.spaceId !== undefined && st2.spaceId !== null) ? st2.spaceId : st2.currentBox;
    if (visitedId !== undefined && visitedId !== null) stat.visited[String(visitedId)] = true;
...
function runSelftest() {
  ...
  // T3 a: 进入 9 号盒（UKE 实例被 AC 直接进入 → 标记：本实例记 0B、交换 #26 同名 0B 实例为 1MB）
  ...
  eq('T3 事件=[enter,mark]（UKE 实例被直接进入 → 标记）', etypes, ['enter', 'mark']);
  ok('T3 落点=右边缘正中 #28', st.player.r === 3 && st.player.c === 6 && st.grid[27].id === 28);
  var sp1 = engine.getSpaceState('1');
  eq('T3 交换: #26 的同名 9号盒 0B→1MB', sp1.shared[25].size, '1.00MB');
  ...
  // 作者语义（2025-XX 定版）：1MB 入口盒在 UKE 空间 → mark(1MB→0B)+交换后仍按 1MB-孪生进入
  //   → teleport 到 twin 帧、栈裁剪至 twin 所在深度（与 25/1MB 进入、29/1MB 进入同构）。
  ok('T7a 进盒2', engine.move('d') === true);
  st = engine.getState();
  eq('T7 进入盒2（teleport 帧：栈裁至 twin 深度）', [st.spaceId, st.pos, st.boxStack, st.moves], [2, { r: 4, c: 1 }, [1, 2], 4]);
  ...
  // T9 链: d d d d d d（用户规则1 + 统一递归）：d1/d2 推链；d3 链末越界推不出 → 最远可进入实例 → AC 进入；
  //   d4-d6 boxEnter（盒进入盒，其余前进一格）。
  var res = [], sawWin = false, sawBoxEnter = false;
  for (var k = 0; k < 6; k++) {
    res.push(engine.move('d'));
    if (engine.getState().events.some(function (e) { return e.type === 'win'; })) sawWin = true;
    if (engine.getState().events.some(function (e) { return e.type === 'boxEnter'; })) sawBoxEnter = true;
  }
  eq('T9 链: 6次连续（统一递归后全通）', res, [true, true, true, true, true, true]);
  st = engine.getState();
  ok('T9 盒进入盒事件（boxEnter）', sawBoxEnter);

  // T10 undo×5 → 撤销 5 个成功步回到 T9d1 之后（moves=8, 2ms (4,5)）
  for (var u = 0; u < 5; u++) engine.undo();
  st = engine.getState();
  eq('T10 undo 回到 T9d1 后', [st.spaceId, st.pos, st.moves, st.won], [2, { r: 4, c: 5 }, 8, false]);

  // T11 interact: 复位后 ',' 进盒2(TLE) → ',' 退出 → 删除当前实例并立即开辟新初始实例
  engine.reset();
  ...
  ok('T11b interact 退出（回 1ms：模板下 2号盒为初始 0B，非孪生进入）', engine.interact() === true);
  st = engine.getState();
  eq('T11b interact 退出回 1ms', [st.spaceId, st.pos, st.boxStack, st.moves], [1, { r: 4, c: 2 }, [1], 2]);
  var sp2 = engine.getSpaceState('2');
  eq('T11 退出后：删除当前实例并开辟新实例（layers=1）', sp2.layers.length, 1);
