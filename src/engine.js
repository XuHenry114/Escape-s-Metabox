...
(function (root, factory) {
  ...
  var VERSION = '2.1.1';   // 2.1.1: 0 悖论——虚空（刷新盒保留 UKE 0B → 其退出直达 egg1）
  var WIN_SPACE = 50;
...
var EPS_SPACE = 'egg3';    // ε 悖论——无限进入 = egg3 地图
  ...
  Engine.prototype._adoptVoidDoor = function (fresh, doorKeep) {
    ...
    var b = doorKeep.box;
    b._voidRetained = true;
    b.pos = { r: posR, c: posC };
    if (cell && cell.box) cell.box = b;                  /* 原位替换模板刚物化的同格实例 */
    else if (fresh.grid[posR]) fresh.grid[posR][posC] = { box: b, baseLabel: (cell && cell.baseLabel) || 'UKE' };
    else { fresh.grid[posR] = []; fresh.grid[posR][posC] = { box: b, baseLabel: 'UKE' }; }
    this._voidDoor = b;
    this._voidLabel = String(b.spaceId);
  };
  ...
  Engine.prototype._canPush = function (g, chain, dir) {
    ...
    if (!inGrid(nr, nc)) {
      ...
      if (this.options.pushOut !== false && g !== this.root) {
        ...
        var laCk = this._landAt(outT.pg, { r: pr, c: pc }, dir, 0);
        return laCk.type === 'boxEnter';
      }
      return false;
    }
  ...
  Engine.prototype._exitOutTarget = function (g, dir, edgeCell) {
    ...
    if (g.kind === 'shared' && mmG && mmG.type === 'UKE') {
      ...
      if (cpG) {
        ...
        var cellG = g.grid[tG.r] && g.grid[tG.r][tG.c];
        if (cellG && cellG.box === null) return { pg: g, rc: tG, cyclic: true, partner: cpG.uid };
      ...
      if (outP) {
        ...
        if (targetCell && targetCell.box) {
          ...
          if (this._canPush(pgOut, pChain, dir)) {
            ...
            var laOut = this._landAt(pgOut, to, dir, 0);
        ...
        if (depth < 0 && tg) {
          ...
          if (foundDepth >= 0) {
            ...
            for (var ciT = 0; ciT < chainFrames.length; ciT++) {
              this.frames.push({ view: chainFrames[ciT].view, parent: chainFrames[ciT].parent, boxUid: chainFrames[ciT].uid, boxPos: chainFrames[ciT].pos, spaceId: String(chainFrames[ciT].view.spaceId) });
            }
  ...
  Engine.prototype._exitAttempt = function (dir) {
    ...
    if (!boundMode && this._voidDoor && frame.boxUid != null && frame.boxUid === this._voidDoor.uid) {
    ...
    if (viaZeroB) {
      ...
      var cellZ = viaZeroB.home.grid[tZ.r] && viaZeroB.home.grid[tZ.r][tZ.c];
      if (cellZ && cellZ.box === null) {
        this._snapshot();
        var leftVZ = this.current;
        // 弹帧：直到栈顶视图 == B 所在网格（B 网格未开时弹空，落到 B 所在开放网格）
        while (this.frames.length) {
          var topVZ = this.frames[this.frames.length - 1];
          if (topVZ.view === viaZeroB.home) break;
          this.frames.pop();
          if (topVZ.view && topVZ.view.kind === 'layer') this._destroyLayer(topVZ.view, true);
        }
        this.current = viaZeroB.home;
        this.pos = tZ;
        this.moves++;
        this._ev('exit', { dir: dir, to: clonePos(tZ), boxUid: viaZeroB.box.uid, spaceId: +viaZeroB.box.spaceId, cyclic: true });
    ...
    if (cell.box === null) {
      ...
      if (!boundMode && inGrid(inward.r, inward.c)) {
        ...
        if (ic && ic.box && mb && ic.box.type === mb.type) {
          ...
          if (!spaceAlreadyOpen) {
            var leftViewC0 = this.current;
            var resC0 = this._tryEnter(ic.box, dir, { tunnel: true, ownerGrid: boxGrid, exitPos: { r: this.pos.r, c: this.pos.c }, boxPos: clonePos(ic.box.pos) });
            if (resC0) this._refreshTLE(leftViewC0);   // 退出 A（TLE）→ 刷新
            return resC0;
          }
        }
      }
  ...
  Engine.prototype._landAt = function (g, t, dir, depth) {
  ...
  Engine.prototype._execBoxEnter = function (la, dir, g) {
    ...
    if (pView2 && mF2) {
      ...
      var lFb2 = this._resolveLanding(pView2, mF2, dir, null, { boxType: farBox2.type });
      lFree2 = (lFb2 && lFb2.land) || null;
      if (lFree2) {
        // 落点有实例 → 先推（同 AC 落点逻辑）
        if (lFb2.chainToPush) this._pushChain(pView2, lFb2.chainToPush, dir);
        var pvCell = pView2.grid[lFree2.r] && pView2.grid[lFree2.r][lFree2.c];
        if (!pvCell) { if (!pView2.grid[lFree2.r]) pView2.grid[lFree2.r] = []; pView2.grid[lFree2.r][lFree2.c] = { box: null, baseLabel: 'WA' }; pvCell = pView2.grid[lFree2.r][lFree2.c]; }
        pvCell.box = prevBox2;
  ...
  Engine.prototype._ensureAnchor = function () {
    ...
    if (!pgA) return;
    this.frames.push({ view: this.current, parent: pgA, boxUid: ownerA.uid, boxPos: clonePos(ownerA.pos), spaceId: String(this.current.spaceId) });
  };
  ...
  Engine.prototype._cloneWorld = function () {
    ...
    var frames = this.frames.map(function (f) {
      return { view: cloneGrid(f.view), parent: cloneGrid(f.parent), boxUid: f.boxUid, boxPos: clonePos(f.boxPos), land: f.land ? clonePos(f.land) : null, spaceId: f.spaceId, master: !!f.master, entryKind: f.entryKind || null, boundGrid: f.boundGrid ? cloneGrid(f.boundGrid) : null };
    });
    return {
  ...
  Engine.prototype._restoreWorld = function (s) {
    ...
    this.frames = s.frames.map(function (f) {
      return { view: f.view, parent: f.parent, boxUid: f.boxUid, boxPos: clonePos(f.boxPos), land: f.land ? clonePos(f.land) : null, spaceId: f.spaceId, master: !!f.master, entryKind: f.entryKind || null, boundGrid: f.boundGrid || null };
    });
    this.moves = s.moves;
    this.won = s.won;
